...
public class UserDao
{
	...
	public boolean login(User user)
	{
		...
		conn = DbConn.getConn();									
		String sql = "SELECT * FROM SALESMAN WHERE SNAME=? AND SPASSWORD=?";  //引用saleman表 数据库很离奇。当前创建表的就不行，不明原因！ 
		...
		try
		{
			...
			while (rs.next())
			{
				...
				String[] hobbys = {""};
					...
					{
						hobbys = rs.getString(7).split("&"); //将字符串分割盛到数组中去  很不理解，取出的String转成String[]存储,显示时却用string。
					}
